...
def discriminator_model():
    ...
    dense1 = Dense(8, activation='relu')(categorical_input)

    inputL = Input(shape=(32, 32, 3), name='image')
    conv1 = Conv2D(50, kernel_size=(3, 3), strides=(1, 1), padding='same', activation='relu')(inputL)
    conv2 = Conv2D(75, kernel_size=(3, 3), strides=(1, 1), padding='same', activation='relu')(conv1)
    maxP1 = MaxPool2D(pool_size=(2, 2))(conv2)
    bn = BatchNormalization()(maxP1)
    do1 = Dropout(0.25)(bn)
    conv3 = Conv2D(125, kernel_size=(3, 3), strides=(1, 1), padding='same', activation='relu')(do1)
    maxP2 = MaxPool2D(pool_size=(2, 2))(conv3)
    do2 = Dropout(0.25)(maxP2)
    dense2 = Dense(32, activation='relu')(do2)
    flatten2 = Flatten()(dense2)

    merge = concatenate([dense1, flatten2])

    dense3 = Dense(1028, activation='relu')(merge)
    dropout1 = Dropout(0.4)(dense3)
    dense4 = Dense(514, activation='relu')(dropout1)
    dropout2 = Dropout(0.4)(dense4)
    dense5 = Dense(256, activation='sigmoid')(dropout2)
    dropout3 = Dropout(0.4)(dense5)
    dense6 = Dense(128, activation='sigmoid')(dropout3)
    dropout4 = Dropout(0.4)(dense6)
